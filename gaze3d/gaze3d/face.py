...
class FaceTracker:
    ...
    def process(self, frame_bgr: np.ndarray, t: float) -> FaceState | None:
        ...
        # MediaPipe VIDEO mode requires strictly increasing ms timestamps.
        # Pushed frames can arrive close enough together that int()-truncated
        # ms collide, which MediaPipe treats as a hard error rather than a
        # warning. Clamp instead of trusting the raw delta.
        ts_ms = max(int((t - self._t0) * 1000), self._last_ts_ms + 1)
