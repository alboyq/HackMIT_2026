...
from arm_executor import run_arm_delivery
...
async def ai_status():
    # vision_provider()/tts_provider() do blocking network reachability probes
    # (urllib.request.urlopen) -- offload so a slow/unreachable local service
    # (e.g. no SSH tunnel to Ollama/Kokoro right now) can't freeze the whole
    # event loop, which every other in-flight request (including arm.target)
    # shares.
    vision, tts = await asyncio.to_thread(lambda: (vision_provider(), tts_provider()))
    return {"vision": vision, "tts": tts}
...
async def websocket_endpoint(ws: WebSocket):
    ...
    try:
        ...
        while True:
            ...
            # arm.target (PRD section 11: orchestrator -> arm) additionally
            # kicks off the real delivery; arm.status progress is broadcast
            # from inside run_arm_delivery as the remote script runs.
            if msg.get("type") == "arm.target" and isinstance(msg.get("object_id"), int):
                asyncio.create_task(run_arm_delivery(msg["object_id"], hub))
