...
export type HubMessage =
  ...
  // Section 11: orchestrator -> arm. Sent by a page when a selection locks
  // in; the backend runs the real teach-and-replay executor and reports
  // progress back as arm.status.
  | { type: 'arm.target'; object_id: number; label?: string }
  // arm -> dashboard. `stage` is either a replay.py stage name (hover, grasp,
  // lift, place, release, home) or one of QUEUED / DONE / FAILED / BUSY.
  | { type: 'arm.status'; object_id: number; stage: string; message?: string }
...
export function useHubSocket(onMessage: (msg: HubMessage) => void) {
  ...
  const send = useCallback((msg: HubMessage) => {
