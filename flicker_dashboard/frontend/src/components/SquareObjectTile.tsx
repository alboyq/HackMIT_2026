      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/60 to-transparent px-4 pb-4 pt-12">
        <div className="flex items-end justify-between gap-2">
          <div className="min-w-0 flex-1">
            <div
              className={`font-display font-semibold leading-tight tracking-wide text-white drop-shadow-lg ${
                nameSize === 'sm' ? 'text-[11px] sm:text-xs' : 'text-xl sm:text-2xl'
              }`}
            >
              {title}
            </div>
          ...
          {(locked || (looking && !fetching)) && (
            <span className="shrink-0 rounded bg-[var(--accent)]/90 px-2 py-1 font-mono text-[10px] font-bold tracking-widest text-white shadow-lg">
