...
export default function SpokenCaption({ className = '' }: { className?: string }) {
  ...
  return (
    <div className={`pointer-events-none absolute inset-x-0 bottom-3 z-30 flex justify-center px-4 ${className}`}>
      <p className="max-w-[42rem] rounded-full border border-[var(--accent)]/40 bg-black/75 px-5 py-2 text-center font-mono text-[12px] leading-snug tracking-wide text-white shadow-[0_0_24px_var(--accent-glow)]">
        {line}
      </p>
    </div>
  )
