...
from .solver import ArmIK, ToolFrame
from .scene import HOME_Q
...
class Expert:
    ...
    def grasp_pose(self, name):
        ...
        if strategy == "top":
            # grasp at the object's mid-height, but never so low the pads hit the table
            z = max(0.022, min(p[2], o.rest_z))
    ...
    def plan_present(self, target, approach_dir=None, standoff=STAGING, q_from=None, grip=None):
        ...
        wps = []
        if via[2] > p0[2] + 1e-3:
            seg, why = self._cart_segment(q0, p0, via, held_dir, grip, "raise", jaw_dir=held_jaw)
            if seg is not None:
                wps += seg
                q0 = wps[-1].q
            else:
        ...
        for f in (0.0, 0.25, 0.4, 0.55, 0.7, 1.0):
            ...
            seg, why = self._cart_segment(q0, via, stage, held_dir, grip, "carry",
                                          jaw_dir=held_jaw, tool_dir_to=d_to, min_steps=24)
            if seg is not None:
