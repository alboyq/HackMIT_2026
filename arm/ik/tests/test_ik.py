...
from arm.controller import IKController, select_controller
...
def test_pick_and_lift(name):
    ...
    assert z1 - z0 > 0.08, f"{name} rose only {(z1 - z0) * 1000:.0f} mm (strategy {plan.strategy})"


def test_minimum_jerk_limits(scene):
    ik = ArmIK(scene.model)
    q = minimum_jerk(ik.lo + 0.3, ik.hi - 0.3, 30.0, 0.5, 1.5)
    assert validate_trajectory(q, ik.lo, ik.hi, 30.0, 0.5, 1.5) is None
