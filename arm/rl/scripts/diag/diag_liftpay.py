...
wins = 0
for ep in range(N):
    obs = env.reset(); seated_steps = 0; tp = None; took = False; maxlift = 0; info = {}; rs = []; ph = Counter(); pin = []; seat = []
    for i in range(300):
        ...
        obs, r, done, infos = env.step(a); info = infos[0]; e_rew = r
        if not took:
            ...
            if not done[0]:
                rs.append(float(e_rew[0])); ph[info['phase']] += 1; pin.append(info['pinched']); seat.append(info['seat_frac'])
        if done[0]: break
    if not took:
    ...
    print(f"{info['object']:6s} {1000*info['object_width_m']:5.0f}  {tp:6d}   {f0:8.1f}                     {maxlift:8.0f}     {str(not info['pinched']):5s}   "
          f"{1000*info['stage_drift_m']:7.1f}   {info['peak_grip_force_n']:8.1f}   {info['success']}  | lift-phase reward/step {np.mean(rs) if rs else 0:+.3f} (sum {np.sum(rs):+.1f}) pinched {np.mean(pin) if pin else 0:.0%} seat {np.mean(seat) if seat else 0:.2f} phases {dict(ph)}")
